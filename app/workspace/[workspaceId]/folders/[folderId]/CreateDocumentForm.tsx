...
export function CreateDocumentForm({
    ...
    async function handleSubmit(e: FormEvent) {
        ...
        const parsed = CreateDocumentSchema.shape.title.safeParse(title);
        if (!parsed.success) {
        ...
        try {
            const res = await fetch(
                `/api/workspaces/${workspaceId}/folders/${folderId}/documents`,
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ title: parsed.data }),
                }
            );
            const data = await res.json().catch(() => ({}));

            if (!res.ok) {
                setError(data.error ?? "Couldn't create the document. Please try again.");
                return;
            }

            showToast(`Document "${parsed.data}" created`);
            router.push(
                `/workspace/${workspaceId}/documents/${data.document.id}/edit`
            );
        } catch {
        ...
        >
            <div className="flex flex-col gap-1">
                <input
                    ...
                    className="h-10 w-56 rounded-md border border-black/[.12] bg-transparent px-3 text-sm text-zinc-950 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 dark:border-white/[.15] dark:text-zinc-50 dark:focus:border-indigo-400"
                />
                {error && (
